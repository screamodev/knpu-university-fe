...
useHead({
  title: () => t('nav.links.benefits'),
  meta: [{ name: 'description', content: () => t('admissions.benefits.subtitle') }],
})
  ...
  <div class="bg-white min-h-screen">
    ...
    <div class="bg-gradient-to-b from-navy-deep to-navy py-16">
      <div class="max-w-container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-[11px] font-semibold tracking-wider uppercase text-gold/90 mb-3">
          {{ t('admissions.benefits.tag') }}
        </div>
        <h1 class="font-playfair text-3xl md:text-4xl font-bold text-white">
          {{ t('admissions.benefits.title') }}
        </h1>
        <p class="mt-4 text-white/70 max-w-2xl">
          {{ t('admissions.benefits.subtitle') }}
        </p>
      </div>
    </div>
    ...
    <div class="bg-off-white border-t border-border py-12 lg:py-16">
      <div class="max-w-container mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="font-playfair text-2xl font-bold text-navy mb-8">
          {{ t('admissions.benefits.categoriesTitle') }}
        </h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div
            v-for="key in categoryKeys"
            :key="key"
            class="bg-white border border-border rounded-16 p-6 flex flex-col gap-3"
          >
            ...
            <h3 class="font-playfair text-lg font-semibold text-navy">
              {{ t(`admissions.benefits.${key}`) }}
            </h3>
            <p class="text-body-sm text-text-muted">
              {{ t(`admissions.benefits.${key}Desc`) }}
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-container mx-auto px-4 sm:px-6 lg:px-8 py-14 lg:py-20">
      <h2 class="font-playfair text-2xl font-bold text-navy mb-10">
        {{ t('admissions.benefits.howToApplyTitle') }}
      </h2>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        >
          ...
          <h3 class="font-playfair text-lg font-semibold text-navy mb-2">
            {{ t(`admissions.benefits.${step.titleKey}`) }}
          </h3>
          <p class="text-body text-text-muted">
            {{ t(`admissions.benefits.${step.textKey}`) }}
          </p>
        </div>
      </div>
    </div>
